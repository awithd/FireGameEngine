#include "FGEEditMeshSelector.h"


FGEEditMeshSelector::FGEEditMeshSelector()
{
...
void FGEEditMeshSelector::init(OpenGLFunctions *f, FGEDataProject *struct_project, FGEGLSL *shader)
{
    this->struct_project=struct_project;
    this->shader = shader;
    this->shader->clearAttribLocation();
...
void FGEEditMeshSelector::setSelectMode(int mode){
    this->mode=mode;
}

void FGEEditMeshSelector::setRenderDevice(FGEOpenGLSceneRenderer *render_device)
{
...
bool FGEEditMeshSelector::pressSelect(void *slf, OpenGLFunctions *f, glm::vec3 veiw_pos, FGELight *light_tool, glm::vec2 cursor, FGEDataCameraView *view, FGEDataCameraProjection *projection, GLuint shader_program_selection, QOpenGLContext *oglc, FGERenderTarget *render_tarjet_selection, GLuint WIDTH, GLuint HEIGHT, FGEStateOpperations *opperations_state){
    ...
    if(this->mode==SELECT_DEFAULT_STRATEGY){
        ...
        FGEDataSelectedNodes * sn = this->struct_project->getSelectedNodes();
        for(int _i=0; _i<sn->sizeNodes(); _i++){
            FGEDataNode *node = sn->getNode(_i);
            if(this->render_device!=NULL){
                glm::mat4 model = node->transform->getGlobalTransformation();
                glm::mat4 view_projection_matrix = projection->matrix*view->matrix;
                std::vector<glm::mat4> mbl;
                this->render_device->device_mesh->paintSkinPointEdgeFace(
                            f,
                            _i*100000,
        ...
        for(int _i=0; _i<sn->sizeNodes(); _i++){
            ...
            if(res[0]==0 && res[1]==0 && res[2]==0){
                ...
                if(getSelectedObjects(node, _i*100000, opperations_state->mesh_select, res[0], res[1], res[2])){
                    st = true;
                }
            }
        }
...
bool FGEEditMeshSelector::radar(OpenGLFunctions *_f, int strategy, glm::vec2 cursor, glm::vec3 veiw_pos, FGEDataCameraView *view, FGEDataCameraProjection *projection, QOpenGLContext *oglc, FGERenderTarget *render_tarjet_selection, FGEStateOpperations *opperations_state){
    ...
    if(strategy==SELECT_RECT_STRATEGY){
        ...
        render_tarjet_selection->bind(_f);
            ...
            FGEDataNode *current_node = NULL;///sns->getCurrentNode();
...
void FGEEditMeshSelector::moveSelect(OpenGLFunctions *f, glm::vec2 cursor, GLuint WIDTH, GLuint HEIGHT){
    if(this->drag){
...
bool FGEEditMeshSelector::releazeSelect(void *slf, OpenGLFunctions *_f, glm::vec2 cursor, FGELight *light_tool, FGEDataCameraView *view, FGEDataCameraProjection *projection, GLuint shader_program_selection, QOpenGLContext *oglc, FGERenderTarget *render_tarjet_selection, GLuint WIDTH, GLuint HEIGHT, FGEStateOpperations *opperations_state)
{
    if(this->drag){
        this->drag = false;
        this->draw_st=false;
...
void FGEEditMeshSelector::scaleSelected(int f, unsigned char *pixels){
    int ii = 0;
    for(int i=0; i<f; i++){
        bool st = false;
        for(int j=0; j<this->points.size(); j++){
...
void FGEEditMeshSelector::drawArea(OpenGLFunctions *f){
...
bool FGEEditMeshSelector::isDrug(){
    if(this->draw_st){
        if(this->mode==SELECT_RECT_STRATEGY){
            return true;
        }else if(this->mode==SELECT_CIRCLE_STRATEGY){
...
void FGEEditMeshSelector::drawSelectionObjects(OpenGLFunctions *f, FGEDataNode *node,  glm::vec3 veiw_pos, FGEDataCameraView *view, FGEDataCameraProjection *projection, FGEStateOpperations *opperations_state){
    ...
    if(this->render_device!=NULL){
        ...
        //this->render_device->draw(); f, node, opperations_state->mesh_select);
...
bool FGEEditMeshSelector::getSelectedObjects(FGEDataNode *node, int index, int mode, int r, int g, int b){
    ...
    if(node!=NULL){
        if(mode==1){
            if(node->selection_edit_mode->appendPoint(id)){
                qDebug() << "---1yes";
                return true;
            }
        }else if(mode==2){
            if(node->selection_edit_mode->appendLine(id)){
                qDebug() << "---2yes";
                return true;
            }
        }else if(mode==3){
            if(node->selection_edit_mode->appendFace(id)) {
                qDebug() << "---3yes";
                return true;
            }
        }
...
void FGEEditMeshSelector::drawBoxArea(OpenGLFunctions *f)
{
