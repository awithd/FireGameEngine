#ifndef FGESELECTIONARMATUREPAINTWEIGHT_H
#define FGESELECTIONARMATUREPAINTWEIGHT_H
...
class FGESelectionArmaturePaintWeight : public QOpenGLFunctions
{
public:
    FGESelectionArmaturePaintWeight();
    ...
    void scaleSelected(int f, unsigned char *pixels, int radius);
    //void drawSelectionObjects(FGENodeOS *nd, FGELigthTool *light_tool);
    bool getSelectedComposes(int r, int g, int b, bool sp);
    void drawBoxArea(OpenGLFunctions *f);
    bool getSelectedPoints(FGEDataNode *node, int r, int g, int b);

    bool radar(OpenGLFunctions *_f, int radius, int strategy, glm::vec2 cursor, glm::vec3 veiw_pos, FGEDataCameraView *view, FGEDataCameraProjection *projection, QOpenGLContext *oglc, FGERenderTarget *render_tarjet_selection, FGEStateOpperations *opperations_state);
...
#endif // FGESELECTIONARMATUREPAINTWEIGHT_H
