#include "FGEGeometryEditBoxForm.h"
#include "ui_FGEGeometryEditBoxForm.h"

FGEGeometryEditBoxForm::FGEGeometryEditBoxForm(FGEDataProject *data_project, FGEOpenGLWidget *glwidget_world, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FGEGeometryEditBoxForm)
{
    ui->setupUi(this);
    ...
    size = this->sizeHint();
    QObject::connect(animation, &QPropertyAnimation::valueChanged, this, &FGEGeometryEditBoxForm::slotValueChanged);
    ...
    if(sn->selected_nodes.size()==1){
        ...
        if(node!=NULL){
            if(node->mesh!=NULL){
                if(node->mesh->shape_type.type==FGE_SHAPE_TYPE_BOX){
                    this->shape_type = node->mesh->shape_type.type;
                    FGE3DGeometryItemBox *item_box = (FGE3DGeometryItemBox *)node->mesh->shape_type.item;
                    ui->spinBox_x->setValue(item_box->sub_width);
                    ui->spinBox_y->setValue(item_box->sub_height);
                    ui->spinBox_z->setValue(item_box->sub_depth);
                    ui->doubleSpinBox_x->setValue(item_box->width);
                    ui->doubleSpinBox_y->setValue(item_box->height);
                    ui->doubleSpinBox_z->setValue(item_box->depth);
...
FGEGeometryEditBoxForm::~FGEGeometryEditBoxForm()
{
    delete ui;
}

void FGEGeometryEditBoxForm::on_pushButton_clicked()
{
    this->pass_arrow = false;
    if(swap){
...
void FGEGeometryEditBoxForm::slotValueChanged(const QVariant &value)
{
        ...
        if(node!=NULL){
            if(node->mesh!=NULL){
                if(node->mesh->shape_type.type==FGE_SHAPE_TYPE_BOX){
                    ...
                    box.updateNodeBoxSubdevide(this->glwidget_world->openGLFunctions(), this->data_project, node, node->mesh, w, h, z, sub_x, sub_y, sub_z);
...
void FGEGeometryEditBoxForm::on_doubleSpinBox_x_valueChanged(double arg1)
{
    this->updateBox();
}

void FGEGeometryEditBoxForm::on_doubleSpinBox_y_valueChanged(double arg1)
{
    this->updateBox();
}

void FGEGeometryEditBoxForm::on_doubleSpinBox_z_valueChanged(double arg1)
{
    this->updateBox();
}

void FGEGeometryEditBoxForm::on_spinBox_x_valueChanged(int arg1)
{
    this->updateBox();
}

void FGEGeometryEditBoxForm::on_spinBox_y_valueChanged(int arg1)
{
    this->updateBox();
}

void FGEGeometryEditBoxForm::on_spinBox_z_valueChanged(int arg1)
{
    this->updateBox();
}
