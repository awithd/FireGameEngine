#ifndef FGEEDITMESHSELECTOR_H
#define FGEEDITMESHSELECTOR_H
...
/**
 * @brief The FGEEditMeshSelector class provides functionality for selecting points, lines,
 * or faces within a mesh, depending on the active editing mode.
 *
 * FGEEditMeshSelector allows users to interactively choose and manipulate specific components
 * of a mesh through mouse clicks. This class is particularly valuable in 3D modeling or editing
 * applications, enhancing the ability to perform precise edits on 3D models.
 */

class FGEEditMeshSelector : public QOpenGLFunctions
{
public:
    FGEEditMeshSelector();
    ...
    void drawSelectionObjects(OpenGLFunctions *f, FGEDataNode *node, glm::vec3 veiw_pos, FGEDataCameraView *view, FGEDataCameraProjection *projection, FGEStateOpperations *opperations_state);
    ...
    bool getSelectedObjects(FGEDataNode *node, int index, int mode, int r, int g, int b);
