#include "FGESelectionArmatureEditable.h"

FGESelectionArmatureEditable::FGESelectionArmatureEditable()
{
...
void FGESelectionArmatureEditable::init(OpenGLFunctions *f, FGEDataProject *struct_project, FGEGLSL *shader)
{
    this->struct_project=struct_project;
    this->shader = shader;
    this->shader->clearAttribLocation();
...
void FGESelectionArmatureEditable::setSelectMode(int mode){
    this->mode=mode;
}

void FGESelectionArmatureEditable::setRenderDevice(FGEOpenGLSceneRenderer *render_device)
{
...
bool FGESelectionArmatureEditable::radar(OpenGLFunctions *_f, int strategy, glm::vec2 cursor, glm::vec3 veiw_pos, FGEDataCameraView *view, FGEDataCameraProjection *projection, QOpenGLContext *oglc, FGERenderTarget *render_tarjet_selection, FGEStateOpperations *opperations_state){
...
void FGESelectionArmatureEditable::moveSelect(OpenGLFunctions *f, glm::vec2 cursor, GLuint WIDTH, GLuint HEIGHT){
    if(this->drag){
...
void FGESelectionArmatureEditable::scaleSelected(int f, unsigned char *pixels){
    int ii = 0;
    for(int i=0; i<f; i++){
        bool st = false;
        for(int j=0; j<this->points.size(); j++){
...
void FGESelectionArmatureEditable::drawArea(OpenGLFunctions *f){
...
bool FGESelectionArmatureEditable::isDrug(){
    if(this->draw_st){
        if(this->mode==SELECT_RECT_STRATEGY){
            return true;
        }else if(this->mode==SELECT_CIRCLE_STRATEGY){
...
void FGESelectionArmatureEditable::drawSelectedArmatures(OpenGLFunctions *f, FGEDataNode *node,  glm::vec3 veiw_pos, FGEDataCameraView *view, FGEDataCameraProjection *projection, FGEStateOpperations *opperations_state){
...
bool FGESelectionArmatureEditable::getSelectedComposes(int r, int g, int b, bool sp){
...
void FGESelectionArmatureEditable::drawBoxArea(OpenGLFunctions *f)
{
